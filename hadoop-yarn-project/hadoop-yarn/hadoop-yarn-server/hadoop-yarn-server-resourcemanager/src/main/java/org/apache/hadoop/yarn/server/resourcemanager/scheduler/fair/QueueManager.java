...
public class QueueManager {
    ...
    private Collection<FSParentQueue> parentQueues = new CopyOnWriteArrayList<>();
    QueueSortThread queueSortThread = new QueueSortThread();

    public QueueManager(FairScheduler scheduler) {
        this.scheduler = scheduler;
        queueSortThread.start();
    }
     ...
     *
     * @return the created queue, if successful. null if not allowed (one of the parent
     ...
     * 和createQueue方法要做好同步。
     */
    class QueueSortThread extends Thread {

        ExecutorService sortService = Executors.newFixedThreadPool(10);

        public void run() {
            while (!Thread.currentThread().isInterrupted()) {
                try {
                    TimeUnit.SECONDS.sleep(1);
                    parentQueues.stream().forEach(new Consumer<FSParentQueue>() {
                        @Override
                        public void accept(final FSParentQueue fsParentQueue) {
                            sortService.submit(new Runnable() {
                                public void run() {
                                    fsParentQueue.sort();
                                }
                            });
